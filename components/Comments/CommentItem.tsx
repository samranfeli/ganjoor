...
const CommentItem: React.FC<Props> = props => {
    const { comment } = props;
    return (
        <div key={comment.id} className={`border border-stone-200 bg-white p-2 md:p-3 text-justify rounded my-2 md:my-4 ${props.isReply ? "mr-1 md:mr-8" : ""}`}>

            <div className='md:flex md:flex-row-reverse justify-between mb-3'>
                <div dir="ltr" className='text-stone-400 text-sm'>
